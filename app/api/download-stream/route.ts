...
const resetStatusStmt = db.prepare('UPDATE videos SET status = ?, downloaded = 0 WHERE id = ?');
...
// Background queue to prevent DB writes from blocking the stream
const updateQueue = new Map<number, { status: string, downloaded: number }>();
...
// Helper to update DB status efficiently
function updateStatus(id: number, status: string, downloaded: number) {
    updateQueue.set(id, { status, downloaded });
    if (!isFlushing) {
        isFlushing = true;
        setImmediate(flushUpdateQueue);
    }
}
...
export async function GET(req: NextRequest) {
    ...
    if (!video) {
        ...
        resetStatusStmt.run('downloading', video.id);
    }
